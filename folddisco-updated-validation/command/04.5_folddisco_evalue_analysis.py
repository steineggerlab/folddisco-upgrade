...
DATA_DIR = "result_whole/folddisco_results_analyses"
RESULT_DIR = "result_whole/folddisco_results_evalues"
...
PLOT_DIR = "result_whole/analyses_plot"
...
def plot_roc_curve(qlen, y_true, y_score, save_path):
    """
    y_true = ground truth  (1 = positive, 0 = negative)
    y_score = -evalue (bigger = more significant)
    """
    # threshold sweeping
    ...
    plt.title(f"ROC Curve (length={qlen}) AUC={auc:.4f}")
    plt.grid(True)
    plt.tight_layout()
...
def plot_pr_curve(qlen, y_true, y_score, save_path, F1):
    # positive sorted high→low
    thresholds = np.unique(y_score)
    thresholds = np.sort(thresholds)
    ...
    plt.title(f"PR Curve (length={qlen})")
    plt.grid(True)
...
for qlen in sorted(matches_by_qlen.keys(), key=lambda x: int(x)):
    ...
    ROC_FILE = os.path.join(PLOT_DIR, f"ROC_metric{METRIC_NUM}_{METRICS[METRIC_NUM]}_len{qlen}_{EVAL_CUTOFF}.png")
    PR_FILE  = os.path.join(PLOT_DIR, f"PR_metric{METRIC_NUM}_{METRICS[METRIC_NUM]}_len{qlen}_{EVAL_CUTOFF}.png")
...
'''
    ...
    # ------------------------
    y_true_self = np.array([1 if m["is_self"] else 0 for m in match_list])
    y_true  = np.array([1 if (m["is_self"] or m["is_similar"]) else 0 for m in match_list])
    y_score = np.array([-m["evalue"] for m in match_list])   # smaller evalue → higher score

    # ------------------------
    # ROC curve
    # ------------------------
    plot_roc_curve(qlen, y_true_self, y_score, ROC_FILE.replace(".png", "_self.png"))
    plot_roc_curve(qlen, y_true, y_score, ROC_FILE)

    # ------------------------
    # PR curve
    # ------------------------
    plot_pr_curve(qlen, y_true_self, y_score, PR_FILE.replace(".png", "_self.png"), F1_self)
    plot_pr_curve(qlen, y_true, y_score, PR_FILE, F1)
'''
log_and_print("===== CONFUSION MATRIX + ROC/PR/F1 DONE =====")

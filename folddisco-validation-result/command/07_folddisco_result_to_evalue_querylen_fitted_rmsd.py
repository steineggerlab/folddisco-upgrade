...
def per_row_output(file_path, out_path):
    ...
    with open(file_path, "r") as f:
        ...
        for line in f:
            ...
            key = cols[1]
            try:
                score = -math.log(float(cols[5]) + 1e-300)
            except ValueError:
                continue
    ...
    with open(out_path, "w") as out:
        out.write("query_num\tsource\tid\tnode_count\trmsd\te_value\n")
        for cols in rows:
            if len(cols) < 5:
                continue
            key = cols[1]
            ...
            e = len(buckets[key]) * p
            out.write(f"{cols[0]}\t{cols[1]}\t{cols[2]}\t{cols[3]}\t{cols[5]}\t{e:.3e}\n")
    ...
    if p_by_len_self or p_by_len_nonself:
        ...
        for L in sorted(set(list(p_by_len_self.keys()) + list(p_by_len_nonself.keys()))):
            ...
            # 안전한 변환 (underflow 방지)
            p_self_L = np.clip(p_by_len_self[L], 1e-50, 1.0)
            p_nonself_L = np.clip(p_by_len_nonself[L], 1e-50, 1.0)

            # --- self ---
            if len(p_self_L) > 0:
                has_any = True
                neglog_self = -np.log10(p_self_L)
                hist_self, bin_edges = np.histogram(neglog_self, bins=bins, density=True)
            ...
            if len(p_nonself_L) > 0:
                ...
                hist_nonself, bin_edges = np.histogram(neglog_nonself, bins=bins, density=True)
                bin_centers = 0.5 * (bin_edges[:-1] + bin_edges[1:])
            ...
            if has_any:
                ...
                plt.legend()
                plt.tight_layout()
                out_png = os.path.join(PNG_DIR, f"pvalue_distribution_L{L}_rmsd.png")
...
def main():
    ...
    with Pool(processes=32) as pool:
        ...
        for file in os.listdir(RESULT_DIR):
            ...
            RESULT_FILE = os.path.join(RESULT_DIR, f"total_evalues_fitted_rmsd.txt")

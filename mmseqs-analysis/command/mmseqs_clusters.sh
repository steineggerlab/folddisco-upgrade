...
for pdb in data/pdbs/*.pdb; do
    id=$(basename "$pdb" .pdb)
    chain=$(pdb_tofasta "$pdb" | grep '^>' | sed 's/^.*|\(.\).*/\1/')
    pdb_tofasta "$pdb" | awk -v id="$id" -v chain="$chain" '
        BEGIN {OFS=""}
        /^>/ {next}
        {
            print ">" id
...
mmseqs createdb result/all_seqs.fasta windows_db
mmseqs cluster windows_db windows_cluster tmp_dir -c $COVERAGE --min-seq-id 0.2 -e $EVALUE
mmseqs createtsv windows_db windows_db windows_cluster result/nonwindows_clusters_"$EVALUE"_"$COVERAGE"_"$WINDOW_SIZE"_"$STEP_SIZE".tsv
